...
public class Downloader {
    ...
    private boolean checkExisting(Dependency dep) {
        ...
        for (File f : files) {
            ...
            if (cmp < 0) {
                System.out.println("[Constitution] Deleted old version " + f.getName());
                deleteDep(f);
                return false;
            }
            if (cmp > 0) {
                System.out.println("[Constitution] Warning: version of " + dep.getName() + ", " + vFile.getVersion() + " is newer than request " + dep.getVersion());
            }
    ...
    private void deleteDep(File depFile) {
        if (!depFile.delete()) {
            ...
            System.out.println("[Constitution] Was not able to delete file " + depFile.getPath() + ". Will try to delete on exit.");
            System.exit(1);
        }
    }
    ...
    private void download(Dependency dep) {
        ...
        try {
            ...
            connection.setRequestProperty("User-Agent", "Constitution Downloader");
