...
public class DependencyLoader {
    ...
    public DependencyLoader(File rootDir, LaunchClassLoader cl) {
    ...
    private void injectDeps() {
        ...
        for (File f : rootDir.listFiles(jarFilter)) {
            ...
            try {
                ...
                System.out.println("[Constitution] Loaded library file " + f.getAbsolutePath());
            } catch(MalformedURLException e) {
                throw new RuntimeException("[Constitution Core] Could not add library file " + f.getAbsolutePath() + ", there may be a class loading problem");
            }
        }
    }
    ...
    private void scanDepInfo(File file) {
        try {
            ...
            System.err.println("[Constitution] Failed to load deps.info from " + file.getName() + " as JSON");
            e.printStackTrace();
        }
    }
    ...
    private void parseJsonArray(JsonArray root) {
        for (JsonElement node : root) {
            if (node.isJsonObject()) {
                ...
                System.out.println("[Constitution] Invalid node in deps.info!");
            }
        }
    }
    ...
    private void parseJson(JsonObject node) {
        ...
        try {
            ...
            System.err.println("[Constitution] Invalid filename pattern: "+node.get("pattern"));
            e.printStackTrace();
        }
        if (pattern == null)
        ...
        if (!versionedFile.matches())
            throw new RuntimeException("[Constitution] Invalid filename format for dependency: " + filename);
